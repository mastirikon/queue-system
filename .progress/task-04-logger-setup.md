...
- [x] Создан файл `pkg/logger/logger.go` с корректным содержимым
- [x] Код компилируется без ошибок: `go build ./pkg/logger`
- [x] Тест production логгера выполнен успешно
- [x] Тест development логгера выполнен успешно
- [x] Результаты тестов показаны ментору
...
### Production логгер (JSON):
```json
{"level":"info","timestamp":"2026-01-08T18:17:56.617+0300","caller":"queue-system/test_logger_prod.go:12","msg":"Production logger test","env":"production","port":8080}
{"level":"error","timestamp":"2026-01-08T18:17:56.617+0300","caller":"queue-system/test_logger_prod.go:17","msg":"Test error message","error":"connection refused","retry":3,"stacktrace":"..."}
```

✅ JSON формат корректный  
✅ Timestamp в ISO8601  
✅ Caller указан  
✅ Stacktrace только для ERROR  

### Development логгер (консоль):
```
2026-01-08T18:19:08.250+0300    DEBUG   queue-system/test_logger_dev.go:12      Debug message (только в dev)
2026-01-08T18:19:08.251+0300    INFO    queue-system/test_logger_dev.go:14      Development logger test {"env": "development", "port": 8080}
2026-01-08T18:19:08.251+0300    WARN    queue-system/test_logger_dev.go:19      Warning message {"reason": "high memory usage"}
2026-01-08T18:19:08.251+0300    ERROR   queue-system/test_logger_dev.go:23      Error message   {"error": "timeout"}
```

✅ DEBUG уровень работает  
✅ Читаемый формат  
✅ Цветной вывод  
✅ Stacktrace для ERROR  

**Замечаний нет!** Идеальная реализация! 🎉
